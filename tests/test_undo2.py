...
def modentry():
    return ModEntry(enabled=1, name="Mod Name", modid=15602,
                    version="v1.0.1",
...
def tracker():
    ...
    return undo.RevisionTracker(ModEntry, lambda e: e.directory,
                                *ModEntry._fields,
                                attrsetter=testattrsetter)
...
def random_mock_modentry():
    ...
    me.name = " ".join(
        random.sample(me.directory, random.randint(1, 4)))
    me.directory = " ".join(me.directory)
    me.modid = random.randrange(50000)
    _v1 = random.choice([""] * 15 + ["v"] * 5 + ["b"])
    _v2 = ".".join([str(random.randint(0, 20)) for _ in
                    range(random.randint(1, 3))])
    me.version = _v1 + _v2
    me.ordinal = random.randint(1, 255)

    return ModEntry(me.enabled, me.name, me.modid, me.version,
                    me.directory, me.ordinal)
...
def test_tracker_init_withargs(new_modentry):
    t = undo.RevisionTracker(ModEntry, lambda e: e.directory,
                             *ModEntry._fields)
...
def test_tracker_init(modentry, tracker: RevTrak):
    ...
    assert tracker._get_delta_field[id(tracker.undostack)](
        d) == "previous"
    assert tracker._get_delta_field[id(tracker.redostack)](
        d) == "current"
...
def test_push_more(modentry, tracker: RevTrak):
    ...
    tracker.push(modentry,
                 undo.Delta(tid, "version", modentry.version, "v2.0"))
...
def test_push_group(modentry, tracker: RevTrak):
    ...
    # dict
    dgroup = [dgroup_dict(id=modentry.directory, attrname=p[0],
                          prev=getattr(current, p[0]), curr=p[1])
              for p in
              dgroup_params]
    assert len(dgroup) == 3
    ...
    check_obj_state(modentry.directory, tracker, 15602, 0, 255,
                    "vWhat.0", "Super Awesome Name")

    tracker.undo(1)
    check_obj_state(modentry.directory, tracker, 15602, 0, 100,
                    "v2.0", "Rather Boring Name")
    ...
    # dict no prev
    dgroup = [
        dgroup_dict(id=modentry.directory, attrname=p[0], curr=p[1])
        for p in
        dgroup_params]
    tracker.push(current, dgroup)
    check_obj_state(modentry.directory, tracker, 15602, 0, 255,
    ...
    ## tuple no prev
    dgroup = [
        dgroup_tuple(id=modentry.directory, attrname=p[0], curr=p[1])
        for p in
        dgroup_params]
    tracker.push(current, dgroup)
    check_obj_state(modentry.directory, tracker, 15602, 0, 255,
    ...
    ## tuple
    dgroup = [dgroup_tuple(id=modentry.directory, attrname=p[0],
                           prev=getattr(current, p[0]), curr=p[1])
              for p in
              dgroup_params]
    tracker.push(current, dgroup)
    ...
    ## delta
    dgroup = [dgroup_delta(id=modentry.directory, attrname=p[0],
                           prev=getattr(current, p[0]), curr=p[1])
              for p in
              dgroup_params]
    tracker.push(current, dgroup)
...
def test_undo(modentry, tracker):
    ...
    check_obj_state(modentry.directory, tracker, modentry.modid,
                    modentry.enabled, modentry.ordinal,
                    modentry.version, modentry.name)
...
def tracker2():
    ...
    track.registerType(type(random_mock_modentry()),
                       lambda v: v.directory, *ModEntry._fields,
                       attrsetter=testattrsetter)
...
def test_track_multiple(randentries, tracker2: RevTrak):
    ...
    for entry, change in zip(entries, changes):
        ...
        ovals[entry.directory] = {
            attr_: getattr(entry, attr_)}  # save original value
    ...
    assert modif(0).modid == ovals[entries[0].directory][
                                 'modid'] + 111
    assert modif(1).name == ovals[entries[1].directory][
                                'name'] + 'morename'
    assert modif(2).version == 'vABC.D'
    assert modif(3).enabled == int(
        not ovals[entries[3].directory]['enabled'])
    assert modif(4).ordinal == ovals[entries[4].directory][
                                   'ordinal'] + 101
...
## Helpers
##===============================================
def check_obj_state(key, _tracker, x_modid, x_enabled,
...
testwords = ["Dawnguard", "HearthFires", "Unofficial", "Skyrim",
             "Legendary", "Edition", "Patch", "Clothing", "and",
             "Clutter", "Fixes", "Cutting", "Room", "Floor", "Guard",
             "Dialogue", "Overhaul", "Invisibility", "Eyes",
             "Fix", "Weapons", "and", "Armor", "Fixes", "Complete",
             "Crafting", "Overhaul", "Remade", "Realistic",
             "Water", "Two", "Content", "Addon", "Explosive", "Bolts",
             "Visualized", "Animated", "Weapon", "Enchants",
             "Deadly", "Spell", "Impacts", "dD", "-", "Enhanced",
             "Blood", "Main", "Book", "Covers", "Skyrim",
             "Improved", "Combat", "Sounds", "v2.2", "Bring", "Out",
             "Your", "Dead", "-", "Legendary", "Edition", "The",
             "Choice", "Is", "Yours", "The", "Paarthurnax", "Dilemma",
