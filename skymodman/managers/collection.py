...
class ModCollectionManager(Submanager):
    ...
    def validate_mods(self, managed_mods_list):
        ...
        Handle discrepancies by raising an Exception object containing
        two separate lists:
        ...
        cmanaged = list(m.directory for m in self.managed_mods())

        # make a copy of the list of mods-on-disk since we may be
        # modifying its contents
        dmanaged = managed_mods_list[:]

        not_found = []
        not_listed = []

        if len(cmanaged) > len(dmanaged):
            for mname in dmanaged:
                try:
                    cmanaged.remove(mname)
                except ValueError:
                    not_listed.append(mname)
            not_found = cmanaged

        else:
            for mname in cmanaged:
                try:
                    dmanaged.remove(mname)
                except ValueError:
                    not_found.append(mname)
            not_listed = dmanaged
        ...
        return not (not_listed or not_found or errors_cleared)
