...
class RevisionTracker:
    ...
    # well this was all pointless...
    # def set_attr(self, attr_name, value, *, target=None, target_id=None, target_type=None):
        # """
        # This call can take two forms:
        #     * ``set_attr(attr_name, value, *, target, target_type=None)``
        # or
        #     * ``set_attr(attr_name, value, *, target_id, target_type)``
        #
        # Either `target` or `target_id` must be provided as a required keyword-only argument. If both are provided, `target_id` is ignored UNLESS `target_type` is not ``None``, in which case `target` is ignored.  `target_type` is a required argument when only `target_id` is provided, but optional for `target`: if `target` is provided and `target_type` is not ``None``, type lookup for `target` will  be skipped and the provided `target_type` used instead.
        #
        # :param attr_name:
        # :param value:
        # :param target:
        # :param target_type: If provided, type lookup will be skipped for the target and
        # :return:
        # """
        #
        #
        # for case in [lambda v: v is not None]:
        #
        #     if case(target_type):
        #         if case(target_id):
        #             self.attr_setters[target_type](
        #                     target_id,
        #                     attr_name,
        #                     value)
        #             break
        #         if case(target):
        #             self.attr_setters[target_type](
        #                     self.getID(target, target_type),
        #                     attr_name,
        #                     value)
        #             break
        #         #for...else: will run if we get here
        #     elif case(target):
        #         ttype = type(target)
        #         self.attr_setters[ttype](self.getID(target, ttype),
        #                                  attr_name, value)
        #         break
        #     elif case(target_id):
        #         ttype = type(self.getTarget(target_id))
        #         self.attr_setters[ttype](target_id, attr_name, value)
        #         break
        # else:
        #     raise TypeError("set_attr() requires one of the keyword-only arguments 'target' or 'target_id'")
        ...
        #         self.attr_setters[target_type](self.getID(target, target_type), attr_name, value)
        #     else:
        #         raise TypeError("set_attr() requires one of the keyword-only arguments 'target' or 'target_id'")
        # else:
        #
